  ...
  if((depRows||[]).length > 0) {
    var depRowsHTML = (depRows||[]).map(function(d) {
      ...
      return '<div style="display:flex;justify-content:space-between;align-items:center;padding:7px 0;border-bottom:1px solid var(--border)22">'
        ...
        + (d.status==='held' ? '<button onclick="quickRefundDeposit(\'' + d.id + '\')" style="padding:5px 9px;background:var(--red)22;border:1px solid var(--red);border-radius:8px;color:var(--red);font-size:.72rem;cursor:pointer;font-family:inherit" title="'+(LANG==='ar'?'استرداد التأمين':'Refund Deposit')+'">↩️</button>' : '')
