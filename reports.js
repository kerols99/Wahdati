...
async function loadMonthly(btn) {
  ...
  try{
    ...
    units.forEach(function(u){
      var _effectiveRent = calcEffectiveRent(u, discMap, adjustMap, monYM);
    ...
    units.forEach(function(u){
      ...
      var _aptEffectiveRent = calcEffectiveRent(u, discMap, adjustMap, monYM);
    ...
    floorKeys.forEach(function(floorKey){
      ...
      fl.apts.forEach(function(apt){
        ...
        var rows = g.units.slice().sort(function(a,b){return Number(a.room)-Number(b.room);}).map(function(u){
          ...
          var isNew    = u._isNew;
          var effectiveRent = calcEffectiveRent(u, discMap, adjustMap, monYM);
...
async function exportPDF(type, mon) {
  ...
  try {
    ...
    units.forEach(function(u){
      ...
      var eff = calcEffectiveRent(u, _pdfDiscMap, _pdfAdjustMap, monYM);
      totalRent += eff;
    ...
    units.forEach(function(u){
      ...
      apts[apt].rent     += calcEffectiveRent(u, _pdfDiscMap, _pdfAdjustMap, monYM);
...
// ══════════════════════════════════════════════════════
async function loadVacantActualReport(btn) {
  ...
  try {
    ...
    allUnits.forEach(function(u) {
      ...
      if(!lastTenant) {
        // مفيش مستأجر سابق خالص — شاغرة طول الشهر
      ...
      results.push({
        ...
        lastEndDate: lastTenant ? (lastTenant.end_date||'—') : '—',
        vacantFrom:  vacantFrom,
        daysVacant:  daysVacant,
        lostRent:    estimatedRent,
        isEstimated: !lastTenant
      });
    });
    ...
    window._vacantActualResults = results;
    window._vacantActualMonth = monYM;

    var out = document.getElementById('rVacantOut');
    if(!out) { toast('rVacantOut not found','err'); return; }

    if(!results.length) {
      out.innerHTML = '<div style="text-align:center;color:var(--muted);padding:30px">لا توجد وحدات شاغرة فعلياً في '+monYM+'</div>';
      return;
    }
    ...
    var html = '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:16px">'
      ...
      +'<div style="font-size:.68rem;color:var(--muted)">إجمالي وحدات شاغرة فعلياً</div></div>'
    ...
    html += '<div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:.75rem">'
      ...
      +'<th style="padding:6px 8px;text-align:right">شقة</th>'
      +'<th style="padding:6px 8px;text-align:right">غرفة</th>'
      +'<th style="padding:6px 8px;text-align:right">آخر مستأجر</th>'
      +'<th style="padding:6px 8px;text-align:center">آخر خروج</th>'
      +'<th style="padding:6px 8px;text-align:center">شاغرة من</th>'
      +'<th style="padding:6px 8px;text-align:center">أيام</th>'
      +'<th style="padding:6px 8px;text-align:center">إيجار ضائع</th>'
      +'</tr></thead><tbody>';

    results.forEach(function(r){
      html += '<tr style="border-bottom:1px solid var(--border)">'
        +'<td style="padding:6px 8px">'+esc(String(r.apartment))+'</td>'
        +'<td style="padding:6px 8px">'+esc(String(r.room))+'</td>'
        +'<td style="padding:6px 8px">'+esc(r.lastTenant)+'</td>'
        +'<td style="padding:6px 8px;text-align:center;font-size:.68rem">'+esc(r.lastEndDate)+'</td>'
        +'<td style="padding:6px 8px;text-align:center;font-size:.68rem">'+esc(r.vacantFrom)+'</td>'
        +'<td style="padding:6px 8px;text-align:center;font-weight:700;color:'+(r.daysVacant>30?'var(--red)':r.daysVacant>7?'var(--amber)':'inherit')+'">'+r.daysVacant+'</td>'
        +'<td style="padding:6px 8px;text-align:center">'+r.lostRent.toLocaleString()+(r.isEstimated?' <span style="color:var(--muted);font-size:.65rem">(تقديري)</span>':'')+'</td>'
        +'</tr>';
    });

    html += '<tr style="background:var(--panel2);font-weight:700">'
      +'<td colspan="6" style="padding:6px 8px;text-align:right">الإجمالي</td>'
      +'<td style="padding:6px 8px;text-align:center">'+totalLostRent.toLocaleString()+'</td>'
      +'</tr>';
    ...
    toast('خطأ: '+e.message,'err');
    console.error('loadVacantActualReport:', e);
  } finally {
    if(btn) { btn.disabled=false; btn.innerHTML=orig; }
  }
}
...
// ══════════════════════════════════════════════════════
// REPORT 2: خروج آخر الشهر / المتاح للشهر القادم
// ══════════════════════════════════════════════════════
async function loadEndOfMonthReport(btn) {
  ...
  try {
    ...
    var results = (hist||[]).filter(function(h){
      ...
      return {
        ...
        rent:         h.monthly_rent || 0,
        endDate:      h.end_date,
        availableFrom: monNextStart,
        phone:        h.phone || '—',
        notes:        h.snapshot_type === 'internal_transfer_out' ? 'نقل داخلي' : (h.notes || '')
      };
    });
    ...
    window._endOfMonthResults = results;
    window._endOfMonthMonth = monYM;

    var out = document.getElementById('rEndOfMonthOut');
    if(!out) { toast('rEndOfMonthOut not found','err'); return; }

    if(!results.length) {
      out.innerHTML = '<div style="text-align:center;color:var(--muted);padding:30px">لا يوجد خروج في آخر '+monYM+'</div>';
      return;
    }
    ...
    var html = '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:16px">'
      +'<div style="background:var(--amber)15;border:1px solid var(--amber)33;border-radius:10px;padding:12px;text-align:center">'
      +'<div style="font-size:1.8rem;font-weight:800;color:var(--amber)">'+results.length+'</div>'
      +'<div style="font-size:.68rem;color:var(--muted)">وحدة ستصبح متاحة</div></div>'
      +'<div style="background:var(--accent)15;border:1px solid var(--accent)33;border-radius:10px;padding:12px;text-align:center">'
      +'<div style="font-size:1.4rem;font-weight:800;color:var(--accent)">'+totalRent.toLocaleString()+'</div>'
      +'<div style="font-size:.68rem;color:var(--muted)">إجمالي الإيجار (AED)</div></div>'
      +'</div>';
    ...
    html += '<div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:.75rem">'
      ...
      +'<th style="padding:6px 8px;text-align:right">شقة</th>'
      +'<th style="padding:6px 8px;text-align:right">غرفة</th>'
      +'<th style="padding:6px 8px;text-align:right">المستأجر</th>'
      +'<th style="padding:6px 8px;text-align:center">الإيجار</th>'
      +'<th style="padding:6px 8px;text-align:center">تاريخ الخروج</th>'
      +'<th style="padding:6px 8px;text-align:center">متاح من</th>'
      +'<th style="padding:6px 8px;text-align:right">ملاحظات</th>'
      +'</tr></thead><tbody>';
    ...
    results.forEach(function(r){
      html += '<tr style="border-bottom:1px solid var(--border)">'
        +'<td style="padding:6px 8px">'+esc(String(r.apartment))+'</td>'
        +'<td style="padding:6px 8px">'+esc(String(r.room))+'</td>'
        +'<td style="padding:6px 8px">'+esc(r.tenant)+'</td>'
        +'<td style="padding:6px 8px;text-align:center">'+r.rent.toLocaleString()+'</td>'
        +'<td style="padding:6px 8px;text-align:center;font-size:.68rem">'+esc(r.endDate)+'</td>'
        +'<td style="padding:6px 8px;text-align:center;font-size:.68rem">'+esc(r.availableFrom)+'</td>'
        +'<td style="padding:6px 8px;font-size:.68rem;color:var(--muted)">'+esc(r.notes)+'</td>'
        +'</tr>';
    });

    html += '<tr style="background:var(--panel2);font-weight:700">'
      +'<td colspan="3" style="padding:6px 8px;text-align:right">الإجمالي</td>'
      +'<td style="padding:6px 8px;text-align:center">'+totalRent.toLocaleString()+'</td>'
      +'<td colspan="3"></td>'
      +'</tr>';
    html += '</tbody></table></div>';
    ...
    toast('خطأ: '+e.message,'err');
    console.error('loadEndOfMonthReport:', e);
  } finally {
    if(btn) { btn.disabled=false; btn.innerHTML=orig; }
  }
}
...
// ══════════════════════════════════════════════════════
// REPORT 3: النقل الداخلي خلال الشهر
// ══════════════════════════════════════════════════════
async function loadInternalTransfersReport(btn) {
  var monEl = document.getElementById('rit-month');
  if(!monEl || !monEl.value) { toast(LANG==='ar'?'اختر الشهر':'Choose month','err'); return; }
  var monYM    = monEl.value.slice(0,7);
  var monStart = monYM+'-01';
  var monEnd   = window.monthEnd(monYM);
  ...
  try {
    ...
    var results = transfersOut.map(function(h){
      // ابحث عن سجل internal_transfers يطابق from_unit_id
      var match = transfersTbl.find(function(t){ return t.from_unit_id === h.unit_id; });
      var toApt = '—', toRoom = '—', newRent = '—';
      if(match) {
        try {
          var toSnap = typeof match.to_snapshot === 'string' ? JSON.parse(match.to_snapshot) : match.to_snapshot;
          toApt = toSnap.apartment || '—';
          toRoom = toSnap.room || '—';
          newRent = toSnap.monthly_rent || '—';
        } catch(_e) {}
      }
      return {
        tenant:    h.tenant_name || '—',
        ...
        toApt:     toApt,
        toRoom:    toRoom,
        transferDate: h.end_date,
        oldRent:   h.monthly_rent || 0,
        newRent:   newRent,
        deposit:   h.deposit || 0,
        notes:     h.notes || ''
      };
    ...
    window._internalTransfersResults = results;
    window._internalTransfersMonth = monYM;

    var out = document.getElementById('rInternalTransfersOut');
    if(!out) { toast('rInternalTransfersOut not found','err'); return; }

    if(!results.length) {
      out.innerHTML = '<div style="text-align:center;color:var(--muted);padding:30px">لا يوجد نقل داخلي في '+monYM+'</div>';
      return;
    }

    var esc = function(v){ return String(v||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;'); };

    var html = '<div style="display:grid;grid-template-columns:1fr;gap:10px;margin-bottom:16px">'
      +'<div style="background:var(--accent)15;border:1px solid var(--accent)33;border-radius:10px;padding:12px;text-align:center">'
      +'<div style="font-size:1.8rem;font-weight:800;color:var(--accent)">'+results.length+'</div>'
      +'<div style="font-size:.68rem;color:var(--muted)">عمليات نقل داخلي</div></div>'
      +'</div>';

    html += '<div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:.75rem">'
      +'<thead><tr style="background:var(--panel2)">'
      +'<th style="padding:6px 8px;text-align:right">المستأجر</th>'
      +'<th style="padding:6px 8px;text-align:center">من</th>'
      +'<th style="padding:6px 8px;text-align:center">إلى</th>'
      +'<th style="padding:6px 8px;text-align:center">تاريخ النقل</th>'
      +'<th style="padding:6px 8px;text-align:center">الإيجار القديم</th>'
      +'<th style="padding:6px 8px;text-align:center">الإيجار الجديد</th>'
      +'<th style="padding:6px 8px;text-align:center">التأمين</th>'
      +'</tr></thead><tbody>';

    results.forEach(function(r){
      html += '<tr style="border-bottom:1px solid var(--border)">'
        +'<td style="padding:6px 8px">'+esc(r.tenant)+'</td>'
        +'<td style="padding:6px 8px;text-align:center">'+esc(String(r.fromApt))+'/'+esc(String(r.fromRoom))+'</td>'
        +'<td style="padding:6px 8px;text-align:center">'+esc(String(r.toApt))+'/'+esc(String(r.toRoom))+'</td>'
        +'<td style="padding:6px 8px;text-align:center;font-size:.68rem">'+esc(r.transferDate)+'</td>'
        +'<td style="padding:6px 8px;text-align:center">'+Number(r.oldRent).toLocaleString()+'</td>'
        +'<td style="padding:6px 8px;text-align:center">'+(r.newRent==='—'?'—':Number(r.newRent).toLocaleString())+'</td>'
        +'<td style="padding:6px 8px;text-align:center">'+Number(r.deposit).toLocaleString()+'</td>'
        +'</tr>';
    });
    ...
    toast('خطأ: '+e.message,'err');
    console.error('loadInternalTransfersReport:', e);
  } finally {
    if(btn) { btn.disabled=false; btn.innerHTML=orig; }
  }
}
...
// ══════════════════════════════════════════════════════
// REPORT 4: المستأجرين الجدد خلال الشهر
// ══════════════════════════════════════════════════════
async function loadNewTenantsReport(btn) {
  var monEl = document.getElementById('rnew-month');
  if(!monEl || !monEl.value) { toast(LANG==='ar'?'اختر الشهر':'Choose month','err'); return; }
  var monYM    = monEl.value.slice(0,7);
  var monStart = monYM+'-01';
  var monEnd   = window.monthEnd(monYM);
  ...
  try {
    ...
    (histStartRes.data||[]).forEach(function(h){
      ...
      // ملاحظة: internal_transfer_out هنا يعني المستأجر دخل في هذا الشهر (start_date)
      // حتى لو خرج لاحقاً بنقل داخلي — هو لا يزال "جديد" بالنسبة لهذا الشهر
    ...
    // تقرير الجدد يعرض "جديد" فقط — المنقولون داخلياً في تقرير النقل
    var newOnly = results.filter(function(r){ return r.source !== 'internal_transfer_in'; });

    window._newTenantsResults = results; // كل النتائج للـ audit
    window._newTenantsMonth = monYM;

    var out = document.getElementById('rNewTenantsOut');
    if(!out) { toast('rNewTenantsOut not found','err'); return; }

    if(!newOnly.length) {
      out.innerHTML = '<div style="text-align:center;color:var(--muted);padding:30px">لا يوجد مستأجرين جدد في '+monYM+'</div>';
      return;
    }
    ...
    var html = '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:16px">'
      +'<div style="background:var(--green)15;border:1px solid var(--green)33;border-radius:10px;padding:12px;text-align:center">'
      +'<div style="font-size:1.8rem;font-weight:800;color:var(--green)">'+newOnly.length+'</div>'
      +'<div style="font-size:.68rem;color:var(--muted)">مستأجر جديد</div></div>'
      +'<div style="background:var(--accent)15;border:1px solid var(--accent)33;border-radius:10px;padding:12px;text-align:center">'
      +'<div style="font-size:1.4rem;font-weight:800;color:var(--accent)">'+totalRent.toLocaleString()+'</div>'
      +'<div style="font-size:.68rem;color:var(--muted)">إيجار شهر الدخول</div></div>'
      +'<div style="background:var(--amber)15;border:1px solid var(--amber)33;border-radius:10px;padding:12px;text-align:center">'
      +'<div style="font-size:1.4rem;font-weight:800;color:var(--amber)">'+totalDeposits.toLocaleString()+'</div>'
      +'<div style="font-size:.68rem;color:var(--muted)">تأمينات محصّلة</div></div>'
      +'</div>';
    ...
    html += '<div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:.75rem">'
      ...
      +'<th style="padding:6px 8px;text-align:right">غرفة</th>'
      +'<th style="padding:6px 8px;text-align:right">المستأجر</th>'
      +'<th style="padding:6px 8px;text-align:center">تاريخ الدخول</th>'
      +'<th style="padding:6px 8px;text-align:center">عدد</th>'
      +'<th style="padding:6px 8px;text-align:center">الإيجار</th>'
      +'<th style="padding:6px 8px;text-align:center">إيجار أول شهر</th>'
      +'<th style="padding:6px 8px;text-align:center">التأمين</th>'
      +'<th style="padding:6px 8px;text-align:center">المصدر</th>'
      +'</tr></thead><tbody>';

    newOnly.forEach(function(r){
      html += '<tr style="border-bottom:1px solid var(--border)">'
        +'<td style="padding:6px 8px">'+esc(String(r.apartment))+'</td>'
        +'<td style="padding:6px 8px">'+esc(String(r.room))+'</td>'
        +'<td style="padding:6px 8px">'+esc(r.tenant)+'</td>'
        +'<td style="padding:6px 8px;text-align:center;font-size:.68rem">'+esc(r.startDate)+'</td>'
        +'<td style="padding:6px 8px;text-align:center">'+r.personsCount+'</td>'
        +'<td style="padding:6px 8px;text-align:center">'+Number(r.monthlyRent).toLocaleString()+'</td>'
        +'<td style="padding:6px 8px;text-align:center">'+(r.firstMonthRent?Number(r.firstMonthRent).toLocaleString():'—')+'</td>'
        +'<td style="padding:6px 8px;text-align:center">'+(r.depositPaid?Number(r.depositPaid).toLocaleString()+' ('+esc(r.depositDate)+')':'—')+'</td>'
        +'<td style="padding:6px 8px;text-align:center;font-size:.65rem">'+(r.source==='internal_transfer_in'?'نقل داخلي':'جديد')+'</td>'
        +'</tr>';
    });
    ...
    toast('خطأ: '+e.message,'err');
    console.error('loadNewTenantsReport:', e);
  } finally {
    if(btn) { btn.disabled=false; btn.innerHTML=orig; }
  }
}
...
function exportVacantActualPDF() {
  ...
  var rows = results.map(function(r){
    return '<tr>'
      ...
      +'<td style="padding:6px 8px;border:1px solid #ddd;text-align:center;font-size:11px">'+esc(r.lastEndDate)+'</td>'
      +'<td style="padding:6px 8px;border:1px solid #ddd;text-align:center;font-size:11px">'+esc(r.vacantFrom)+'</td>'
      +'<td style="padding:6px 8px;border:1px solid #ddd;text-align:center;font-weight:700">'+r.daysVacant+'</td>'
      +'<td style="padding:6px 8px;border:1px solid #ddd;text-align:center">'+r.lostRent.toLocaleString()+(r.isEstimated?' (تقديري)':'')+'</td>'
      +'</tr>';
  }).join('');
  ...
  openPrintWindow(
    '<div style="font-family:Arial,sans-serif;direction:rtl;padding:20px;color:#111">'
    +'<div style="border-bottom:3px solid #1a3a6a;padding-bottom:12px;margin-bottom:16px;display:flex;justify-content:space-between">'
    +'<div><div style="font-size:18px;font-weight:800;color:#1a3a6a">تقرير الشاغر الفعلي</div>'
    +'<div style="font-size:12px;color:#555;margin-top:3px">'+monYM+'</div></div>'
    +'<div style="font-size:11px;color:#888">'+new Date().toLocaleDateString('ar-AE')+'</div></div>'
    ...
    +'<th style="padding:7px 8px;text-align:center;font-size:10px">آخر خروج</th>'
    +'<th style="padding:7px 8px;text-align:center;font-size:10px">شاغرة من</th>'
    +'<th style="padding:7px 8px;text-align:center;font-size:11px">أيام</th>'
    +'<th style="padding:7px 8px;text-align:center;font-size:11px">إيجار ضائع</th>'
    +'</tr></thead><tbody>'+rows+'</tbody>'
    +'<tfoot><tr style="background:#f0f0f0;font-weight:700">'
    +'<td colspan="6" style="padding:7px 8px;border:1px solid #ddd;text-align:right">الإجمالي</td>'
...
function exportEndOfMonthPDF() {
  ...
  var rows = results.map(function(r){
    return '<tr>'
      +'<td style="padding:6px 8px;border:1px solid #ddd">'+esc(String(r.apartment))+'</td>'
      +'<td style="padding:6px 8px;border:1px solid #ddd">'+esc(String(r.room))+'</td>'
      +'<td style="padding:6px 8px;border:1px solid #ddd">'+esc(r.tenant)+'</td>'
      +'<td style="padding:6px 8px;border:1px solid #ddd;text-align:center">'+Number(r.rent).toLocaleString()+'</td>'
      +'<td style="padding:6px 8px;border:1px solid #ddd;text-align:center;font-size:11px">'+esc(r.endDate)+'</td>'
      +'<td style="padding:6px 8px;border:1px solid #ddd;text-align:center;font-size:11px">'+esc(r.availableFrom)+'</td>'
      +'<td style="padding:6px 8px;border:1px solid #ddd;font-size:11px">'+esc(r.notes)+'</td>'
      +'</tr>';
  }).join('');
  ...
  openPrintWindow(
    '<div style="font-family:Arial,sans-serif;direction:rtl;padding:20px;color:#111">'
    +'<div style="border-bottom:3px solid #1a3a6a;padding-bottom:12px;margin-bottom:16px;display:flex;justify-content:space-between">'
    +'<div><div style="font-size:18px;font-weight:800;color:#1a3a6a">تقرير خروج آخر الشهر / المتاح للشهر القادم</div>'
    +'<div style="font-size:12px;color:#555;margin-top:3px">'+monYM+'</div></div>'
    +'<div style="font-size:11px;color:#888">'+new Date().toLocaleDateString('ar-AE')+'</div></div>'
    +'<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:16px">'
    ...
    +'<th style="padding:7px 8px;text-align:right;font-size:11px">غرفة</th>'
    +'<th style="padding:7px 8px;text-align:right;font-size:11px">المستأجر</th>'
    +'<th style="padding:7px 8px;text-align:center;font-size:11px">الإيجار</th>'
    +'<th style="padding:7px 8px;text-align:center;font-size:10px">تاريخ الخروج</th>'
    +'<th style="padding:7px 8px;text-align:center;font-size:10px">متاح من</th>'
    +'<th style="padding:7px 8px;text-align:right;font-size:10px">ملاحظات</th>'
    +'</tr></thead><tbody>'+rows+'</tbody>'
    +'<tfoot><tr style="background:#f0f0f0;font-weight:700">'
    +'<td colspan="3" style="padding:7px 8px;border:1px solid #ddd;text-align:right">الإجمالي</td>'
    +'<td style="padding:7px 8px;border:1px solid #ddd;text-align:center">'+totalRent.toLocaleString()+' AED</td>'
...
function exportInternalTransfersPDF() {
  ...
  var rows = results.map(function(r){
    return '<tr>'
      +'<td style="padding:6px 8px;border:1px solid #ddd">'+esc(r.tenant)+'</td>'
      +'<td style="padding:6px 8px;border:1px solid #ddd;text-align:center">'+esc(String(r.fromApt))+'/'+esc(String(r.fromRoom))+'</td>'
      +'<td style="padding:6px 8px;border:1px solid #ddd;text-align:center">'+esc(String(r.toApt))+'/'+esc(String(r.toRoom))+'</td>'
      +'<td style="padding:6px 8px;border:1px solid #ddd;text-align:center;font-size:11px">'+esc(r.transferDate)+'</td>'
      +'<td style="padding:6px 8px;border:1px solid #ddd;text-align:center">'+Number(r.oldRent).toLocaleString()+'</td>'
      +'<td style="padding:6px 8px;border:1px solid #ddd;text-align:center">'+(r.newRent==='—'?'—':Number(r.newRent).toLocaleString())+'</td>'
      +'<td style="padding:6px 8px;border:1px solid #ddd;text-align:center">'+Number(r.deposit).toLocaleString()+'</td>'
      +'</tr>';
  }).join('');
  ...
  openPrintWindow(
    '<div style="font-family:Arial,sans-serif;direction:rtl;padding:20px;color:#111">'
    +'<div style="border-bottom:3px solid #1a3a6a;padding-bottom:12px;margin-bottom:16px;display:flex;justify-content:space-between">'
    +'<div><div style="font-size:18px;font-weight:800;color:#1a3a6a">تقرير النقل الداخلي</div>'
    +'<div style="font-size:12px;color:#555;margin-top:3px">'+monYM+'</div></div>'
    +'<div style="font-size:11px;color:#888">'+new Date().toLocaleDateString('ar-AE')+'</div></div>'
    ...
    +'<thead><tr style="background:#1a3a6a;color:#fff">'
    +'<th style="padding:7px 8px;text-align:right;font-size:11px">المستأجر</th>'
    +'<th style="padding:7px 8px;text-align:center;font-size:11px">من</th>'
    +'<th style="padding:7px 8px;text-align:center;font-size:11px">إلى</th>'
    +'<th style="padding:7px 8px;text-align:center;font-size:10px">تاريخ النقل</th>'
    +'<th style="padding:7px 8px;text-align:center;font-size:11px">الإيجار القديم</th>'
    +'<th style="padding:7px 8px;text-align:center;font-size:11px">الإيجار الجديد</th>'
    +'<th style="padding:7px 8px;text-align:center;font-size:11px">التأمين</th>'
...
function exportNewTenantsPDF() {
  ...
  var rows = results.map(function(r){
    return '<tr>'
      ...
      +'<td style="padding:6px 8px;border:1px solid #ddd;text-align:center">'+r.personsCount+'</td>'
      +'<td style="padding:6px 8px;border:1px solid #ddd;text-align:center">'+Number(r.monthlyRent).toLocaleString()+'</td>'
      +'<td style="padding:6px 8px;border:1px solid #ddd;text-align:center">'+(r.firstMonthRent?Number(r.firstMonthRent).toLocaleString():'—')+'</td>'
      +'<td style="padding:6px 8px;border:1px solid #ddd;text-align:center">'+(r.depositPaid?Number(r.depositPaid).toLocaleString():'—')+'</td>'
      +'<td style="padding:6px 8px;border:1px solid #ddd;text-align:center;font-size:10px">'+(r.source==='internal_transfer_in'?'نقل داخلي':'جديد')+'</td>'
      +'</tr>';
  }).join('');
  ...
  openPrintWindow(
    '<div style="font-family:Arial,sans-serif;direction:rtl;padding:20px;color:#111">'
    +'<div style="border-bottom:3px solid #1a3a6a;padding-bottom:12px;margin-bottom:16px;display:flex;justify-content:space-between">'
    +'<div><div style="font-size:18px;font-weight:800;color:#1a3a6a">تقرير المستأجرين الجدد</div>'
    +'<div style="font-size:12px;color:#555;margin-top:3px">'+monYM+'</div></div>'
    +'<div style="font-size:11px;color:#888">'+new Date().toLocaleDateString('ar-AE')+'</div></div>'
    +'<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:16px">'
    ...
    +'<th style="padding:7px 8px;text-align:center;font-size:11px">عدد</th>'
    +'<th style="padding:7px 8px;text-align:center;font-size:11px">الإيجار</th>'
    +'<th style="padding:7px 8px;text-align:center;font-size:10px">إيجار أول شهر</th>'
    +'<th style="padding:7px 8px;text-align:center;font-size:11px">التأمين</th>'
    +'<th style="padding:7px 8px;text-align:center;font-size:10px">المصدر</th>'
    +'</tr></thead><tbody>'+rows+'</tbody></table></div>'
  );
}


window.loadVacantActualReport       = loadVacantActualReport;
window.exportVacantActualPDF        = exportVacantActualPDF;
window.loadEndOfMonthReport         = loadEndOfMonthReport;
window.exportEndOfMonthPDF          = exportEndOfMonthPDF;
window.loadInternalTransfersReport  = loadInternalTransfersReport;
window.exportInternalTransfersPDF   = exportInternalTransfersPDF;
window.loadNewTenantsReport         = loadNewTenantsReport;
window.exportNewTenantsPDF          = exportNewTenantsPDF;
...
// كشف حساب مستأجر
// ══════════════════════════════════════════════════════
async function loadTenantStatement(btn) {
  var aptEl  = document.getElementById('rStmt-apt');
  var roomEl = document.getElementById('rStmt-room');
  var fromEl = document.getElementById('rStmt-from');
  var toEl   = document.getElementById('rStmt-to');
  var out    = document.getElementById('rTenantStatementOut');
  ...
  if(!apt||!room) { toast('أدخل رقم الشقة والغرفة','err'); return; }
  if(!from||!to)  { toast('حدد الفترة من/إلى','err'); return; }

  var orig = btn?btn.innerHTML:'';
  if(btn){ btn.disabled=true; btn.innerHTML='<span class="spin"></span>'; }

  try {
    var fromStart = from+'-01';
    var toEnd     = window.monthEnd(to);

    // جلب بيانات الوحدة الحالية
    var { data: unit } = await sb.from('units')
      .select('id,apartment,room,tenant_name,monthly_rent,start_date,phone')
      .eq('apartment', apt).eq('room', room).single();

    if(!unit) { toast('الوحدة غير موجودة','err'); return; }

    // سجل unit_history للوحدة خلال الفترة
    var { data: hist } = await sb.from('unit_history')
      .select('id,tenant_name,monthly_rent,start_date,end_date,snapshot_type,notes')
      .eq('unit_id', unit.id)
      .order('start_date', {ascending: true});

    // الدفعات خلال الفترة
    var { data: pays } = await sb.from('rent_payments')
      .select('id,amount,payment_month,payment_date,payment_method,notes,receipt_no')
      .eq('unit_id', unit.id)
      .gte('payment_month', fromStart)
      .lte('payment_month', toEnd)
      .order('payment_month', {ascending: true});

    // الخصومات
    var { data: discs } = await sb.from('unit_discounts')
      .select('id,discount_amount,start_date,end_date,reason')
      .eq('unit_id', unit.id)
      .lte('start_date', toEnd)
      .gte('end_date', fromStart);

    // التأمينات
    var { data: deps } = await sb.from('deposits')
      .select('id,tenant_name,amount,status,refund_amount,refund_date,deposit_received_date')
      .eq('unit_id', unit.id)
      .order('deposit_received_date', {ascending: true});

    // النقلات الداخلية
    var { data: transfers } = await sb.from('internal_transfers')
      .select('from_unit_id,to_unit_id,transfer_date,from_snapshot,to_snapshot')
      .or('from_unit_id.eq.'+unit.id+',to_unit_id.eq.'+unit.id);

    var esc = function(v){ return String(v||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;'); };

    // بناء قائمة الأشهر
    var months = [];
    var cur = new Date(fromStart);
    var endD = new Date(toEnd);
    while(cur <= endD) {
    ...
    // discMap لكل شهر
    function getDiscount(monYM) {
      var total = 0;
      (discs||[]).forEach(function(d){
        if(d.start_date <= monYM+'-30' && d.end_date >= monYM+'-01') total += (d.discount_amount||0);
      });
      return total;
    }

    // إيجار فعلي لكل شهر — من unit_history أو units
    function getRentForMonth(monYM) {
      var monStart = monYM+'-01', monEnd2 = window.monthEnd(monYM);
      // شوف في unit_history لو فيه سجل نشط في الشهر ده
      var active = (hist||[]).filter(function(h){
        return (!h.start_date || h.start_date <= monEnd2) && (!h.end_date || h.end_date > monStart);
      }).sort(function(a,b){ return (b.end_date||'9999') > (a.end_date||'9999') ? 1 : -1; });
      if(active.length) return active[0].monthly_rent||0;
      // الوحدة الحالية
      var effStart = getEffectiveStartMonth(unit.start_date);
      if(!effStart || effStart <= monYM) return unit.monthly_rent||0;
      return 0;
    }

    // تقارير الدفعات بالشهر
    var paidByMonth = {};
    (pays||[]).forEach(function(p){
      var m = (p.payment_month||'').slice(0,7);
      if(!paidByMonth[m]) paidByMonth[m] = [];
      paidByMonth[m].push(p);
    });
    ...
    // معلومات الوحدة
    html += '<div style="background:var(--accent)12;border:1.5px solid var(--accent)44;border-radius:12px;padding:12px;margin-bottom:14px">'
      +'<div style="font-weight:800;font-size:.95rem;color:var(--accent)">🏢 شقة '+esc(apt)+' — غرفة '+esc(room)+'</div>'
      +'<div style="font-size:.78rem;color:var(--muted);margin-top:4px">'
      +(unit.tenant_name?'👤 '+esc(unit.tenant_name)+'&nbsp;&nbsp;':'')
      +(unit.phone?'📞 '+esc(unit.phone):'')
      +'</div>'
      +'<div style="font-size:.72rem;color:var(--muted);margin-top:2px">الفترة: '+from+' → '+to+'</div>'
      +'</div>';

    // جدول الحركة الشهرية
    var totalRent=0, totalPaid=0, totalDisc=0;

    html += '<table style="width:100%;border-collapse:collapse;font-size:.75rem;margin-bottom:14px">'
      +'<thead><tr style="background:var(--accent)">'
      +'<th style="padding:6px 8px;color:#fff;text-align:right">الشهر</th>'
      +'<th style="padding:6px 8px;color:#fff;text-align:right">الإيجار</th>'
      +'<th style="padding:6px 8px;color:#fff;text-align:right">خصم</th>'
      +'<th style="padding:6px 8px;color:#fff;text-align:right">صافي</th>'
      +'<th style="padding:6px 8px;color:#fff;text-align:right">مدفوع</th>'
      +'<th style="padding:6px 8px;color:#fff;text-align:right">متبقي</th>'
      +'</tr></thead><tbody>';

    months.forEach(function(m, idx){
      var rent = getRentForMonth(m);
      var disc = getDiscount(m);
      var net  = Math.max(0, rent - disc);
      var paid = (paidByMonth[m]||[]).reduce(function(s,p){ return s+(p.amount||0); },0);
      var rem  = net - paid;
      var rowBg = idx%2===0?'var(--panel2)':'var(--panel)';

      totalRent+=rent; totalDisc+=disc; totalPaid+=paid;

      html += '<tr style="background:'+rowBg+';border-bottom:1px solid var(--border)">'
        +'<td style="padding:6px 8px;font-weight:700">'+m+'</td>'
        +'<td style="padding:6px 8px">'+rent.toLocaleString()+'</td>'
        +'<td style="padding:6px 8px;color:var(--amber)">'+(disc>0?'-'+disc:'—')+'</td>'
        +'<td style="padding:6px 8px;font-weight:700">'+net.toLocaleString()+'</td>'
        +'<td style="padding:6px 8px;color:var(--green);font-weight:700">'+paid.toLocaleString()+'</td>'
        +'<td style="padding:6px 8px;color:'+(rem>0?'var(--red)':rem<0?'var(--accent)':'var(--muted)')+';font-weight:'+(rem!==0?'700':'400')+'">'
          +(rem>0?rem.toLocaleString():rem<0?'+'+Math.abs(rem).toLocaleString()+'↪️':'—')
        +'</td>'
        +'</tr>';

      // تفاصيل الدفعات لهذا الشهر
      if((paidByMonth[m]||[]).length>0) {
        (paidByMonth[m]).forEach(function(p){
          html += '<tr style="background:var(--green)08;border-bottom:1px solid var(--border)">'
            +'<td style="padding:3px 8px 3px 16px;color:var(--muted);font-size:.7rem">↳ '+esc((p.payment_date||'').slice(0,10))+'</td>'
            +'<td colspan="3" style="padding:3px 8px;color:var(--muted);font-size:.7rem">'+(p.receipt_no?'🧾 '+esc(p.receipt_no):'')+(p.notes?' · '+esc(p.notes):'')+'</td>'
            +'<td style="padding:3px 8px;color:var(--green);font-size:.7rem">'+Number(p.amount||0).toLocaleString()+'</td>'
            +'<td style="padding:3px 8px;color:var(--muted);font-size:.7rem">'+esc(p.payment_method||'')+'</td>'
            +'</tr>';
        });
      }
    });

    var totalNet = Math.max(0, totalRent - totalDisc);
    var grandRem = totalNet - totalPaid;
    html += '<tr style="background:var(--accent)22;font-weight:800;border-top:2px solid var(--accent)">'
      +'<td style="padding:7px 8px;color:var(--accent)">الإجمالي</td>'
      +'<td style="padding:7px 8px">'+totalRent.toLocaleString()+'</td>'
      +'<td style="padding:7px 8px;color:var(--amber)">'+(totalDisc>0?'-'+totalDisc:'—')+'</td>'
      +'<td style="padding:7px 8px">'+totalNet.toLocaleString()+'</td>'
      +'<td style="padding:7px 8px;color:var(--green)">'+totalPaid.toLocaleString()+'</td>'
      +'<td style="padding:7px 8px;color:'+(grandRem>0?'var(--red)':grandRem<0?'var(--accent)':'var(--green)')+'">'
        +(grandRem>0?grandRem.toLocaleString():grandRem<0?'+'+Math.abs(grandRem).toLocaleString()+'↪️':'✅ مسدّد')
      +'</td>'
      +'</tr></tbody></table>';

    // النقلات الداخلية
    if((transfers||[]).length>0) {
      html += '<div style="font-weight:700;font-size:.78rem;color:var(--accent);margin-bottom:6px">🔄 حركة النقل الداخلي</div>'
        +'<div style="background:var(--panel2);border-radius:10px;padding:10px;margin-bottom:14px">';
      (transfers).forEach(function(t){
        var isIn  = t.to_unit_id   === unit.id;
        var isOut = t.from_unit_id === unit.id;
        var fs = typeof t.from_snapshot==='string'?JSON.parse(t.from_snapshot||'{}'):t.from_snapshot||{};
        var ts = typeof t.to_snapshot  ==='string'?JSON.parse(t.to_snapshot||'{}'):t.to_snapshot||{};
        if(isIn)  html += '<div style="font-size:.75rem;padding:4px 0;border-bottom:1px solid var(--border)">📥 '+(t.transfer_date||'')+' — نُقل <b>إلى هذه الوحدة</b> من '+esc(fs.apartment||'?')+'/'+esc(fs.room||'?')+'</div>';
        if(isOut) html += '<div style="font-size:.75rem;padding:4px 0;border-bottom:1px solid var(--border)">📤 '+(t.transfer_date||'')+' — نُقل <b>من هذه الوحدة</b> إلى '+esc(ts.apartment||'?')+'/'+esc(ts.room||'?')+'</div>';
      });
      html += '</div>';
    }
    ...
    // التأمينات
    if((deps||[]).length>0) {
      html += '<div style="font-weight:700;font-size:.78rem;color:var(--accent);margin-bottom:6px">🔒 التأمينات</div>'
        +'<div style="background:var(--panel2);border-radius:10px;padding:10px">';
      (deps).forEach(function(d){
        html += '<div style="font-size:.75rem;padding:4px 0;border-bottom:1px solid var(--border);display:flex;justify-content:space-between">'
          +'<span>'+esc(d.tenant_name||'—')+' · '+esc(d.deposit_received_date||'')+'</span>'
          +'<span style="color:var(--accent);font-weight:700">'+Number(d.amount||0).toLocaleString()+' AED '
          +(d.status==='refunded'?'<span style="color:var(--red)">↩️ مُرتجع '+Number(d.refund_amount||0).toLocaleString()+'</span>':'<span style="color:var(--muted)">('+esc(d.status)+')</span>')
          +'</span></div>';
      });
      html += '</div>';
    }

    window._stmtHTML = html;
    window._stmtApt = apt; window._stmtRoom = room; window._stmtFrom = from; window._stmtTo = to;
    if(out) out.innerHTML = html;

  } catch(e) {
    toast('خطأ: '+e.message,'err');
    console.error('loadTenantStatement:', e);
  } finally {
    if(btn){ btn.disabled=false; btn.innerHTML=orig; }
  }
}

function exportTenantStatementPDF() {
  var html = window._stmtHTML;
  if(!html) { toast('شغّل التقرير أولاً','err'); return; }
  var title = 'كشف حساب — شقة '+(window._stmtApt||'')+' / '+(window._stmtRoom||'')+' | '+(window._stmtFrom||'')+' → '+(window._stmtTo||'');
  var pdfHtml = '<div style="font-size:1.1rem;font-weight:800;color:#1a3a6a;border-bottom:2px solid #1a3a6a;padding-bottom:8px;margin-bottom:14px">🧾 '+title+'</div>'+html;
